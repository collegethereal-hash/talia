...
interface PetCustomization {
  ...
  hatType: 'none' | 'pirate' | 'crown' | 'flower' | 'chef';
  accessoryType: 'none' | 'scarf' | 'glasses' | 'bow' | 'chef_outfit';
}
...
export const PetHub = () => {
  ...
  const ARCHI_THOUGHTS = {
    hungry: [`Животик урчит... ${p1}, покорми меня!`, `${p2}, я бы не отказался от вкусняшки 🍖`, "Кто-нибудь видел мою миску? Она пуста!"],
  ...
  const ARCHI_HISTORY = `Арчи — не просто пес, он Звездный Хранитель вашего уюта. Рожденный в свете кометы "Сердце Ориона", он приземлился в мире ${spaceConfig?.name || 'Talia'}, чтобы оберегать связь ${p1} и ${p2}. 

Арчи всегда рядом, чтобы напомнить вам о самом важном. Своим золотистым мехом он освещает самые темные уголки, а его виляющий хвост — это компас, который всегда указывает путь друг к другу. Он чувствует каждое ваше доброе слово и каждое проявление заботы, становясь сильнее и мудрее вместе с вашими отношениями.

С каждым новым уровнем (Lvl) Арчи обретает новые силы, а его преданность становится вечным символом вашей любви.`;
          ...
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 md:p-8">
            ...
            <motion.div
              ...
              <div className="w-full md:w-2/5 bg-[#f5e6d3] relative flex flex-col items-center justify-center p-4 md:p-8 border-b md:border-b-0 md:border-r-4 border-[#e6d5bc] shrink-0">
                ...
                <div className="relative w-full aspect-square md:aspect-auto md:flex-1 bg-[#f5e6d3]/30 rounded-[3rem] overflow-visible border-4 border-[#e6d5bc]/50 shadow-inner group">
                  <Archi2D customization={state.customization} isHappy={showHeart} />
                    ...
                    {activeTab === 'closet' && (
                      ...
                      >
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div className="space-y-4">
                              <h4 className="text-sm font-black uppercase tracking-widest text-[#8b7355]">Одежда</h4>
                              <div className="grid grid-cols-2 gap-2">
                                {['none', 'chef_outfit'].map(acc => (
                                  <button 
                                    key={acc}
